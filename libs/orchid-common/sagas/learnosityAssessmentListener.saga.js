...
var _marked = /*#__PURE__*/_regeneratorRuntime.mark(listenForChangesFromLearnosity),
    _marked2 = /*#__PURE__*/_regeneratorRuntime.mark(closeChannelWhenAssignmentIsSubmitted),
    _marked3 = /*#__PURE__*/_regeneratorRuntime.mark(learnosityLiveMessageListener);

import assignConfig from '../utils/assignConfig';
import { startAssignmentFromLearnosity, submitAssignmentFromPostMessage } from '../actions';
import { attemptToParseJson } from '../utils/jsonUtil.util';
import { call, put, take, fork } from 'redux-saga/effects';
import { eventChannel } from 'redux-saga';
import { ASSIGNMENT_ACTIONS } from '../actions/types';
export function listenForChangesFromLearnosity(assignment) {
  var channel, event;
  return _regeneratorRuntime.wrap(function listenForChangesFromLearnosity$(_context) {
    while (1) {
      switch (_context.prev = _context.next) {
        ...
        case 9:
          event = _context.sent;
          _context.next = 12;
          return call(learnosityLiveMessageListener, event, assignment);
...
export function learnosityLiveMessageListener(event, assignment) {
  var msg;
  return _regeneratorRuntime.wrap(function learnosityLiveMessageListener$(_context3) {
    while (1) {
      switch (_context3.prev = _context3.next) {
        case 0:
          ...
          if (!(msg.msgType === assignConfig.msgType.userhasPressedStart && msg.itemRefId === assignment.refId && event.origin === window.location.origin)) {
            _context3.next = 5;
            break;
          }

          _context3.next = 5;
          return put(startAssignmentFromLearnosity(assignment));

        case 5:
          if (!(msg.msgType === assignConfig.msgType.userhasPressedSubmit && msg.itemRefId === assignment.refId && event.origin === window.location.origin)) {
            _context3.next = 8;
            break;
          }
